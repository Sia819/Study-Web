...
let myMember1: number | string; //      여러 타입은 | 기호로 구분
let myMember2: number | string; //    타입이 여러개이면 괄호로 묶어도 ok
...
let myMembers: (number | string)[];
myMembers = [1, "2", 3];

// 주의사항
let myMembers2: number | string[]; // 이와 같이 괄호를 하지 않고 타입을 지정했다면?
myMembers2 = 123;
myMembers2 = ["a", "b", "c"];
...
let myObj1: { data: string };
myObj1 = { data: "abc" };

// 그렇다면 숫자와 문자가 동시에 들어가는 Object는?
let myObj2: { data: number | string };
myObj2 = { data: 123 };
myObj2 = { data: "abc" };
